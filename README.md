...
It's based off of the multithreaded renderer from the master branch which has it's quirks to say the least (surprisingly inconcistent frametimes) but nevertheless is steady enough to use as a benchmark. This will test across a few numbers of threads, higher physically threaded systems are likely to see better performance out of the higher thread run modes (don't be alarmed if 64T runs slower than 16T for example, this is quite typical at least on my system).
